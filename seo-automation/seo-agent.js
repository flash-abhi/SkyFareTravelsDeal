...
const SITE_URL = 'https://skyfaretravelsdeals.com';
...
class SEOAutomationAgent {
  ...
  async checkBacklinks() {
    ...
    try {
      const response = await axios.get(`${API_BASE}/api/seo/backlinks`, {
        params: { domain: 'skyfaretravelsdeals.com' }
      });
  ...
  async generateContentSuggestions() {
    ...
    try {
      const prompt = `As an SEO expert for a travel deals website (skyfaretravelsdeals.com), suggest 5 blog post topics for this week that will:
