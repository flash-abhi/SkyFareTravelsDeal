...
let seoSettings = {
  ...
  metaDescription:
    "Find the best flight deals, hotels, cruises, and vacation packages at unbeatable prices. Book your next adventure with skyfaretravelsdeals.com - Your trusted travel partner.",
  keywords:
    "flight deals, cheap flights, hotels, cruises, vacation packages, travel deals, airline tickets",
  ogImage: "https://skyfaretravelsdeals.com/og-image.jpg",
  canonicalUrl: "https://skyfaretravelsdeals.com",
};
...
let websiteContent = {
  ...
  heroSubtitle: '',
  aboutText: '',
  contactEmail: 'info@skyfaretravelsdeals.com',
...
let gscSettings = {
  ...
  siteUrl: process.env.GSC_SITE_URL || 'https://skyfaretravelsdeals.com',
  enabled: false,
  connected: false
};
...
router.get('/seo/audit', verifyAdminToken, async (req, res) => {
  try {
    const siteUrl = process.env.GSC_SITE_URL || 'https://skyfaretravelsdeals.com';
...
router.post('/blog/ai/generate', verifyAdminToken, async (req, res) => {
  try {
    ...
    const blogUrl = `https://skyfaretravelsdeals.com/blog/${aiPost.slug}`;
